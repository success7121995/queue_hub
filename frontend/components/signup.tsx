...
const Signup = () => {
	...
	const validateStep1 = () => {
		...
		// ⏳ reserve email uniqueness check for next step
	...
	const onNext = async () => {
		...
		if (!await checkEmailUnique()) return;
	...
	return (
		<div className="min-h-screen flex flex-col items-center justify-center bg-white font-regular-eng px-4 sm:px-6 lg:px-8">
			...
			<div className="flex justify-center items-center w-full mt-4 sm:mt-8 mb-2">
				...
				<span className={`text-lg sm:text-xl ml-2 ${step === 2 ? "text-text-main" : "text-gray-400"}`}>2. Address</span>
			</div>
			...
			{/* Right: Form */}
				<div className="w-full lg:flex-1 flex flex-col items-center justify-center px-2 sm:px-4">
					<form onSubmit={onSubmit} className="w-full max-w-sm space-y-4 sm:space-y-6 mb-8 sm:mb-15">
					<h1 className="text-3xl sm:text-4xl text-center mb-4 sm:mb-6 font-semibold text-text-main">{step === 1 ? "Sign Up" : "Address"}</h1>
