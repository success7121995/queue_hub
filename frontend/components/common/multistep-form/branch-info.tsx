...
const BranchInfo: React.FC<BranchInfoProps> = ({ onNext }) => {
    ...
    const handleOpeningHoursChange = (index: number, field: 'openTime' | 'closeTime' | 'closed', value: string | boolean) => {
        const currentHours = getValues('openingHours') || [];
        const dayHours = currentHours.find(h => h.dayOfWeek === index + 1);

        if (dayHours) {
            setValue('openingHours', currentHours.map(h => 
                h.dayOfWeek === index + 1 ? { ...h, [field]: value } : h
            ));
        } else {
            setValue('openingHours', [...currentHours, {
                id: index + 1,
                dayOfWeek: index + 1,
    ...
    return (
        ...
        >
            <div className="w-full max-w-2xl bg-white rounded-2xl shadow-lg p-8 flex flex-col gap-6 border border-gray-100 relative">
                ...
                <div>
                    <label htmlFor="contactPerson" className="block mb-1 font-semibold text-text-main text-sm">Contact Person</label>
                    <select
                        id="contactPerson"
                        className={`w-full border rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary ${
                            errors.contactPerson ? "border-red-500" : "border-gray-400"
                        }`}
                ...
                {/* Branch Tel */}
                <div>
                    <label htmlFor="branchTel" className="block mb-1 font-semibold text-text-main text-sm">Branch Tel</label>
                    <input
                        id="branchTel"
                        type="tel"
                        className={`w-full border rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary ${
                            errors.branchTel ? "border-red-500" : "border-gray-400"
                        }`}
                        {...register("branchTel", { 
                    ...
                    />
                    {errors.branchTel && <span className="text-red-500 text-xs">{errors.branchTel.message}</span>}
                </div>
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day, index) => {
                            const dayHours = watch('openingHours')?.find(h => h.dayOfWeek === index + 1);
                            return (
